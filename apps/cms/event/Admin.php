...
class Admin extends Addon
{
    ...
    {
        return model('cms/Detail','loglic')->fields($data);
    }
    ...
    protected function ajaxJson()
    {
        ...
        //查询数据
        $list = cmsSelect( DcArrayEmpty($args) );
        if( is_null($list) ){
            return [];
        }
        //拼装数据
        foreach($list['data'] as $key=>$value){
            $list['data'][$key]['cms_category'] = implode(',',$value['category_name']);
        }
        return $list;
    }
    ...
    public function create()
    {
        ...
        return $this->fetch('cms@admin/create');
    }
    ...
    public function edit()
    {
        ...
        return $this->fetch('cms@admin/edit');
    }
    ...
    public function preview()
    {
        ...
        //去掉后台入口文件
        $url = str_replace($this->request->baseFile(), '', cmsUrlDetail( cmsGetId($id)) );
        //跳转至前台
