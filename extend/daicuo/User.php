...
class User
{
    ...
    public static function token_login($user_name='', $user_pass='', $user_expire=30)
    {
        ...
        $user = self::get_user_by('user_name', $user_name, false);
        if(!$user){
            self::is_lock(true);
    ...
    public static function token_check($user_token)
    {
        ...
        //查询TOKEN
        $user = self::get_user_by('user_token', $user_token, false);
        if(!$user){
    ...
    {
        return self::update_user_meta($user_id, 'user_expire', 0);//设置过期时间为0
    }
